...
- The flip mirrors the **rotated** frame, not the source.
  A rotate-and-flip therefore lands identically on iOS and Android; before this was pinned by tests, the two disagreed and produced correctly-sized, visibly wrong images.
- Annotations are positioned against the **output** frame -- after the crop and rotation.
  That is the frame the author was looking at, and it means changing the crop does not move a caption out from under them.
...
| `annotations` | See [annotations](./20-annotations.md). |
