...
description: "Requirements, permissions, and the places the two platforms differ."
---
...
**Preview rotation.**
iOS delivers frames already oriented; Android streams them in sensor orientation.
The host applies `PreviewTexture.sensorOrientation` -- see [capture](/monolens/guides/capture/#rendering-the-viewfinder).

**Texture ids.**
Zero is valid. An iPhone reports it for its first session, so test `preview == null` rather than a falsy id.

**Camera cold start.**
Sub-second on a handset. On an Android emulator's virtual scene it can exceed two minutes, which is why the camera tests carry a long timeout.
